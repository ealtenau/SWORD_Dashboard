...
def get_data(fn):
	...
	sword_simple['width'] = np.round(sword['width'], 3)
	sword_simple['dist_out'] = np.round(sword['dist_out'], 3)
	sword_simple['slope'] = np.round(sword['slope'], 3)
...
for ind in list(range(len(shp_paths))): #len(shp_paths); 35 is mississippi basin
	...
	obs = folium.GeoJsonTooltip(fields=["reach_id", "river_name", "swot_obs", "pass_ids"])
	...
	rch_cmap = []
	#originally: branca.colormap.LinearColormap
	wse_cmap = bcm.LinearColormap(
			colors=wse_colors,
			index=wse_bins,
			vmin=wse_bins[0],
			vmax=wse_bins[-1],
	...
	rch_layer = folium.GeoJson(
		...
		highlight_function=lambda feature: {
			...
			"weight": 6,
			"fillOpacity": 1,})
	rch_popup = CustomLatLngPopup()
	rch_layer.add_child(rch_popup)
	...
	wse_layer = folium.GeoJson(
		...
		highlight_function=lambda feature: {
			...
			"weight": 6,
			"fillOpacity": 1,})
	wse_popup = CustomLatLngPopup()
	wse_layer.add_child(wse_popup)
	...
	wth_layer = folium.GeoJson(
		...
		highlight_function=lambda feature: {
			...
			"weight": 6,
			"fillOpacity": 1,})
	wth_popup = CustomLatLngPopup()
	wth_layer.add_child(wth_popup)
	...
	facc_layer = folium.GeoJson(
		...
		highlight_function=lambda feature: {
			...
			"weight": 6,
			"fillOpacity": 1,})
	facc_popup = CustomLatLngPopup()
	facc_layer.add_child(facc_popup)
	...
	dist_layer = folium.GeoJson(
		...
		highlight_function=lambda feature: {
			...
			"weight": 6,
			"fillOpacity": 1,})
	dist_popup = CustomLatLngPopup()
	dist_layer.add_child(dist_popup)
	...
	slope_layer = folium.GeoJson(
		...
		highlight_function=lambda feature: {
			...
			"weight": 6,
			"fillOpacity": 1,})
	slope_popup = CustomLatLngPopup()
	slope_layer.add_child(slope_popup)
	...
	obs_layer = folium.GeoJson(
		...
		control = True,
		show = False,
		popup=folium.GeoJsonPopup(fields=['reach_id', 'river_name', 'swot_obs','pass_ids']),
		highlight_function=lambda feature: {
			"color": "yellow",
			"weight": 6,
			"fillOpacity": 1,})
	obs_popup = CustomLatLngPopup()
	obs_layer.add_child(obs_popup)
	...
	MousePosition(
		...
		lng_first=False,
		num_digits=7,
		prefix="Coordinates (Lat | Lon):",).add_to(parent_map)
